...
#define GROUNDHUMIDITY A0 //5v:[dry=300,humid=700,water=950]; 3,3V=[dry=0,humid=300,water=700]
#define BRIGHTNESS A1 //pin photocell attached
//[Roomlight=ca.50lux =22][
...
int light,temp,humAir,humGround,value;
SoftwareSerial blueToothSerial(RxD,TxD);//the software serial port 

char recv_str[100];

//der Beispielcode haben wir für unsere Anwendung angepasst
...
{ 
    ...
    if(recvMsg(1000) == 0)
    {
      light=analogRead(BRIGHTNESS);
      humGround=analogRead(GROUNDHUMIDITY);
      if(light>140&&humGround<20)value=0;//hell+trocken
      if(light>140&&humGround>20)value=1;//hell+feucht
      if(light<140&&humGround<20)value=2;//dunkel+trocken
      if(light<140&&humGround>20)value=3;//dunkel+feucht
 ...
 // float t = dht.readTemperature();
        ...
        blueToothSerial.println(light);//return back message
        blueToothSerial.println(humGround);//return back message
